...
- `MB3`: key-positions = <18 19> → &mkp 4（中ボタン、K+L同時押し）
...
- `&mkp` を keymap で使う場合、L・R 両側に `CONFIG_ZMK_MOUSE=y` が必要
- `<dt-bindings/zmk/pointing.h>` を keymap に include すると ZMK v0.3 でビルドエラーになる。マウスボタンは数値リテラルで指定すること（例: `&mkp 4` = MB3）
